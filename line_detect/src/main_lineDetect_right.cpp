...
{
	ros::init(argc,argv,"line_detec_node_right");
	ros::NodeHandle nh;
	ros::Publisher pub_right_point = nh.advertise<line_detect::points>("right_point",10);
	line_detect::points right_point;

	image_transport::ImageTransport it(nh);
	image_transport::Publisher img_pub = it.advertise("right_image",1);
    ...
    if(!nh.getParam("fx",fx))fx = 647.295970;
	if(!nh.getParam("fy",fy))fy = 644.093486;
	if(!nh.getParam("cx",cx))cx = 291.287664;
	if(!nh.getParam("cy",cy))cy = 242.008171;
	f = (fx+fy)/2.0;
	...
	img_pub.publish(msg);
		...
		if(ld.lines.size()>0)
		{
            right_point.point1.x = -ld.lines[0][0]+cx;
            right_point.point1.z = -(ld.lines[0][1]-cy);
            right_point.point1.y = -f;
            right_point.point2.x = -ld.lines[0][2]+cx;
            right_point.point2.z = -(ld.lines[0][3]-cy);
            right_point.point2.y = -f;
            right_point.havenopoints = false;
        }
        else
        {
            right_point.point1.x = 0;
            right_point.point1.z =0;
            right_point.point1.y = 0;
            right_point.point2.x = 0;
            right_point.point2.z = 0;
            right_point.point2.y = 0;
            right_point.havenopoints = true;
        }
        pub_right_point.publish(right_point);
        waitKey(20);
		ros::spinOnce();
	}
