...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  try
  {
   ...
   image_from_bag = cv_bridge::toCvShare(msg, "bgr8")->image.clone();
   //imshow("hehe",image_from_bag);
  }
...
{
	ros::init(argc,argv,"line_detect_node_left");
	ros::NodeHandle nh;
	ros::Publisher pub_left_point = nh.advertise<line_detect::points>("left_point",10);

	image_transport::ImageTransport it(nh);
	image_transport::Publisher img_pub = it.advertise("left_image",1);
	sensor_msgs::ImagePtr msg;

	//image_transport::ImageTransport it(nh);
	//image_transport::Subscriber sub = it.subscribe("left_image", 1000000, imageCallback);
 

	line_detect::points left_point;
	int capture_number;
	double thre_angel_;
	double thre_DistanceOfLines_;
	double fx,fy,cx,cy;
	double f;
	if(!nh.getParam("capture_number",capture_number)) capture_number = 1;
	...
	if(!nh.getParam("fx",fx))fx = 647.905652;
	if(!nh.getParam("fy",fy))fy = 645.672596;
	if(!nh.getParam("cx",cx))cx = 306.834941;
	if(!nh.getParam("cy",cy))cy = 266.539969;
	f = (fx+fy)/2.0;
	...
	while (ros::ok())
	{
...

		{
			...
			//imshow("red_binary",binary);
				...
				//imshow("Detected Lines with HoughP_left",image);
...

				{
					...
					left_point.point1.x = -(-ld.lines[0][0]+cx);
					left_point.point1.z = -(ld.lines[0][1]-cy);
					left_point.point1.y = f;
					left_point.point2.x = -(-ld.lines[0][2]+cx);
					left_point.point2.z = -(ld.lines[0][3]-cy);
					left_point.point2.y = f;
					left_point.havenopoints = false;
				}
				else
				{
					left_point.point1.x = 0;
					left_point.point1.z =0;
					left_point.point1.y = 0;
					left_point.point2.x = 0;
					left_point.point2.z = 0;
					left_point.point2.y = 0;
					left_point.havenopoints = true;
				}
				pub_left_point.publish(left_point);
			}
			waitKey(20);
			ros::spinOnce();
		}
