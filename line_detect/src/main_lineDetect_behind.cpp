...
{
	ros::init(argc,argv,"line_detec_node_behind");
	ros::NodeHandle nh;
    ros::Publisher pub_behind_point = nh.advertise<line_detect::points>("behind_point",10);
	line_detect::points behind_point;

	image_transport::ImageTransport it(nh);
	image_transport::Publisher img_pub = it.advertise("behind_image",1);
    ...
    double fx,fy,cx,cy;
	double f;
	if(!nh.getParam("capture_number",capture_number)) capture_number = 2;
    ...
    if(!nh.getParam("fx",fx))fx = 654.671722;
	if(!nh.getParam("fy",fy))fy = 651.489503;
	if(!nh.getParam("cx",cx))cx = 312.011400;
	if(!nh.getParam("cy",cy))cy = 240.743781;
	f = (fx+fy)/2.0;
	VideoCapture cap(capture_number);
	Mat image,roi;//roi_gray;
	...
	while (ros::ok())
	{
		...
		ld.findLines(binary);
        ld.selectGoodLines();
		ld.drawDetectedLines(roi);
		//mshow("Detected Lines with HoughP",image);
	...
	img_pub.publish(msg);
		...
		if(ld.lines.size()>0)
		{
            behind_point.point1.y = -(-ld.lines[0][0]+cx);
            behind_point.point1.z = -(ld.lines[0][1]-cy);
            behind_point.point1.x = -f;
            behind_point.point2.y = -(-ld.lines[0][2]+cx);
            behind_point.point2.z = -(ld.lines[0][3]-cy);
            behind_point.point2.x = -f;
            behind_point.havenopoints = false;
        }
        else
        {
            behind_point.point1.x = 0;
            behind_point.point1.z =0;
            behind_point.point1.y = 0;
            behind_point.point2.x = 0;
            behind_point.point2.z = 0;
            behind_point.point2.y = 0;
            behind_point.havenopoints = true;
        }
        pub_behind_point.publish(behind_point);
