...
#define BUBBLE_TIMES 3                //middle_filter
#define AVO_THRE 1.2
#define STEP 70
...
void laser_scan_callback(const sensor_msgs::LaserScan::ConstPtr& laser)
{
             ...
             for(int i = 0;i < num_readings; i++)
             {
		      ...
		      for(int j = 1;j <= (BUBBLE_TIMES-1); j++)
		             ...
		             for(int k = 1; k <= (BUBBLE_TIMES-j); k++)
                            {
		                    if(all_ranges[BUBBLE_TIMES-k][i] < all_ranges[BUBBLE_TIMES-k-1][i])
                                    {
		                            double temp = all_ranges[BUBBLE_TIMES-k-1][i];
			                    all_ranges[BUBBLE_TIMES-k-1][i] = all_ranges[BUBBLE_TIMES-k][i];
			                    all_ranges[BUBBLE_TIMES-k][i] = temp;
                                            bubble_flag = true;
		                    }
		            }
